...
dlX0_vector = linspace(0, 2, 100);
dlX0        = dlarray(toDevice(dlX0_vector), 'CB');


randIndex = randperm(size(dlX0_vector,2));



dlX0_vector=dlX0_vector(:,randIndex);



dlT0_vector = repmat(1,[1 100]);
dlT0 = dlarray(toDevice(dlT0_vector), 'CB');
...
[dlU0, Ux0] = dlfeval(@modelU_deri,parameters,dlX0,dlT0)


plot(gather(extractdata(dlX0)) , gather(extractdata(Ux0)) ,'-o')
hold on
