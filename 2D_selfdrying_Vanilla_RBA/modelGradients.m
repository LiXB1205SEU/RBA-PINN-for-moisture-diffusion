function [gradients, loss, lossF_weighted, lossF_unweighted, lossU, rsum_new] = modelGradients(parameters, dlX, dlY, dlT, ...
                                            ...
                                            c, epoch, rsum_old, gamma, eta_init, eta_main, rbaScale)

    if nargin < 28 || isempty(gamma); gamma = 0.999; end
    if nargin < 29 || isempty(eta_init); eta_init = 1; end
    if nargin < 30 || isempty(eta_main); eta_main = 0.01; end
    if nargin < 31 || isempty(rbaScale); rbaScale = 1; end
    ...
    % ------------------------------------------------------------------
    f1 = Ut - c_local .* (Uxx + Uyy) - c_local_dh .* (Ux.^2 + Uy.^2) - dhs_dt;
