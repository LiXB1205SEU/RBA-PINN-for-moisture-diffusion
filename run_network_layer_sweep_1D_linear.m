function results = run_network_layer_sweep_1D_linear(opts)
%RUN_NETWORK_LAYER_SWEEP_1D_LINEAR Network-size sweep for the 1D linear BC PINN.
%
% Corrected scope:
%   PDE:        h_t - c h_xx = 0
%   IC/BC:      hard-constrained by the trial solution
%   Reference:  matching analytical solution
...
D.outputRoot = fullfile(rootDir, 'results', '1D_linear_BC');
        ...
        for fold = 1:opts.numFolds
            ...
            if opts.saveTrainedModels
                saveFile = fullfile(trainedRoot, ...
                    sprintf('1D_linear_BC_H%d_N%d_fold%02d_seed%d.mat', hiddenLayers, neurons, fold, seed));
...
save(fullfile(opts.outputRoot, 'network_layer_sweep_1D_linear_BC_results.mat'), 'results', '-v7.3');
end
...
[predictions, references, metricsByTime, metricsTotal] = evaluateModel1D(parameters, opts, useGPU);
...
runResult.xTest = linspace(0, opts.lengthX, opts.numXTest);
...
function [predictions, references, metricsByTime, metricsTotal] = evaluateModel1D(parameters, opts, useGPU)
x = linspace(0, opts.lengthX, opts.numXTest);
nT = numel(opts.tTest);
predictions = zeros(nT, opts.numXTest);
references = zeros(nT, opts.numXTest);

for it = 1:nT
    t = opts.tTest(it);
    XTest = x;
    TTest = t * ones(1, opts.numXTest);
    ...
    references(it,:) = 0.6 + 0.4 .* sin(pi() .* x ./ 2.0) .* exp(-opts.diffusionC .* pi()^2 .* t ./ 4.0);
end
...
for iL = 1:numel(hiddenLayersList)
    ...
    for iN = 1:numel(neuronsList)
        ...
        row = table("1D_linear_BC", hiddenLayers, totalLayers, neurons, sum(idx), ...
...
for iL = 1:numel(hiddenLayersList)
    ...
    for iN = 1:numel(neuronsList)
        ...
        for it = 1:numel(tTest)
            ...
            row = table("1D_linear_BC", hiddenLayers, totalLayers, neurons, t, sum(idx), ...
