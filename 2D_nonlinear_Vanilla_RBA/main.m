...
if exist('paperRunConfig', 'var')
    ...
    if isfield(paperRunConfig, 'rba_gamma'); rba_gamma = paperRunConfig.rba_gamma; end
    if isfield(paperRunConfig, 'rba_eta_init'); rba_eta_init = paperRunConfig.rba_eta_init; end
    if isfield(paperRunConfig, 'rba_eta'); rba_eta = paperRunConfig.rba_eta; end
end
...
for epoch = 1: numEpochs
    ...
    while hasdata (mbq)
        ...
        if isempty(rsum_old) || numel(rsum_old) ~= numel(dlX)
            rsum_old = dlarray(toDevice(zeros(1,numel(dlX))), 'CB');
        end

        [gradients, loss, lossF, lossF_unweighted, lossU, rsum_new] = dlfeval(@modelGradients, parameters, dlX, dlY, dlT,...
                                            ...
                                            c, epoch, rsum_old, rba_gamma, ...
                                            rba_eta_init, rba_eta, rbaScale);
...
rsum_memory_to_save = gatherRsumMemory(rsumMemory);
...
result.rsum_memory = rsum_memory_to_save;
...
for idx = 1:numel(memory)
    if ~isempty(memory{idx})
        values = memory{idx};
        if isa(values, 'dlarray'); values = extractdata(values); end
        values = gather(values);
        sumSquares = sumSquares + sum(double(values(:)).^2);
        count = count + numel(values);
    end
end
if count == 0; scale = 1; else; scale = sqrt(sumSquares / count + 1e-12); end
end

function memory = gatherRsumMemory(memory)
for idx = 1:numel(memory)
    if ~isempty(memory{idx})
        values = memory{idx};
        if isa(values, 'dlarray'); values = extractdata(values); end
        memory{idx} = gather(values);
    end
end
end
